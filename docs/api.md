...
| `TicketQuestionsController` | `/api/tickets/{ticketId}/questions` | `GET` lists a ticket's clarifying-question/decision/failure history (reads `ITicketQuestionRepository` directly, same pattern as `ReviewsController`'s `GET`); `POST /{questionId}/answer` answers the pending question or decision, unblocks the ticket, and re-runs the pipeline detached (`IOrchestrationService.RunPipelineDetached`), returning the unblocked `TicketDto` immediately rather than the eventual pipeline outcome - a decision whose answer is "cancel this" is resolved by the human calling `POST /tickets/{id}/cancel` directly instead, never through this endpoint |
