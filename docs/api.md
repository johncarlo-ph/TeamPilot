...
| `TicketsController` | `/api/projects/{projectId}/tickets`, `/api/tickets/{id}/...` | Ticket board CRUD; `POST /tickets/{id}/start` runs the ticket through the project's configured agent workflow (`OrchestrationService.RunPipelineAsync`, default Research→Design→Coding→Testing) in one call; `POST /tickets/{id}/cancel` abandons a pre-merge ticket (`ToDo`/`InProgress`/`ForReview`) with an optional reason, terminal, no Git side effects |
...
| `Application.Common.Exceptions.WorkflowLockedException` | 409 (a pipeline structure change was attempted while the project has an `InProgress` ticket) |
