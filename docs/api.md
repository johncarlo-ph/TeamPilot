...
| `TicketsController` | `/api/projects/{projectId}/tickets`, `/api/tickets/{id}/...` | Ticket board CRUD; `POST /tickets/{id}/start` runs the ticket through the project's configured agent workflow (`OrchestrationService.RunPipelineAsync`, default Research→Design→Coding→Testing) in one call, returning `Blocked`/`BlockingQuestionId` on the result if a stage asked a question or a Git/LLM failure occurred instead of reaching For Review; `POST /tickets/{id}/cancel` abandons a pre-merge ticket (`ToDo`/`InProgress`/`ForReview`/`Blocked`) with an optional reason, terminal, and deletes its linked branch from Git in the same call if it has one; `POST /tickets/{id}/retry` unblocks a ticket blocked by a Git/LLM failure and re-runs the pipeline (409 if it's blocked by a clarifying question instead - use `TicketQuestionsController` for that) |
...
| `LiveAgentChatController` | `/api/projects/{projectId}/live-agent/messages`, `.../messages/{messageId}/approve-ticket`, `.../messages/{messageId}/reject-ticket` | `GET /messages` returns the project's chat history (oldest first); `POST /messages { content }` sends a user message, runs the Live Agent's bounded tool-use loop, and returns its assistant reply - including `proposedTicketTitle`/`proposedTicketDescription` when the model drafted a ticket. Every message also carries `senderName` (the sending user's display name for a user message, `null` for an assistant message), `createdTicketId` (set once the draft is approved, otherwise `null`), and `ticketRejected` (`true` once dismissed). `POST .../{messageId}/approve-ticket` (no body) creates the real ticket from that message's draft and stamps `createdTicketId` on it in the same call; `POST .../{messageId}/reject-ticket` (no body) dismisses the draft without creating anything, stamping `ticketRejected` instead. Both are idempotent for a repeat of the same decision (just returns current state) and 409 if the *other* decision was already made on that message |
