...
| `AgentsController` | `/api/projects/{projectId}/agents`, `/api/agents/{id}` | Read/update only — list, get, edit configuration/status. No `POST`: all 5 agents (Research/Design/Coding/Testing, plus the standing `LiveAgent`) are system-provisioned per project, never created via the API |
